...
{
  /* FIXME: Write this */
    //printf ("I am in the _adjustHeap function");
    ...
    if (rightIndex < max) {
        smallestIndex = _smallerIndexHeap(heap, leftIndex, rightIndex, compare);
        //if (leftIndex != 0 && rightIndex != 0) {
            if (compare(getDynArr(heap, smallestIndex), getDynArr(heap, pos)) == -1) {
                swapDynArr(heap, pos, smallestIndex);
                _adjustHeap(heap, max, smallestIndex, compare);
            }
        //}
